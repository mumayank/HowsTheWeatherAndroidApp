...
class MainViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun updateFamousCities(searchTerm: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            val reader = BufferedReader(InputStreamReader(getApplication<Application>().resources.openRawResource(
                R.raw.top100cities
            )))
            val arrayList = arrayListOf<CityWithCode>()
            var line = reader.readLine()
            while (line != null) {
                val city = line.substringBefore(",")
                val cityCode = line.substringAfter(",")
                if (searchTerm.isBlank() || city.toLowerCase(Locale.getDefault()).contains(searchTerm.toLowerCase(Locale.getDefault()))) {
